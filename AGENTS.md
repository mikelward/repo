...
- **Watch your own PRs by subscription, plus one scheduled check.** Have a
  ...
  - Settle the fired trigger first thing in the turn, not last. It may
    have silently re-armed rather than retired, so update it rather than
    adding a second chain.
  - Check the fire time you got against the one you asked for -- a
    4-minute request has come back as 64. Re-time it, or say the watch
    isn't armed.
...
- **On every push, update the PR title and body** so they describe the
  full, latest state of the branch -- not the scope it had when it was
  opened. Re-read the diff against `origin/main` and patch whatever
